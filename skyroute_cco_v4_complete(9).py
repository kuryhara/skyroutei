...
if not st.session_state.presentation_mode:
    top_shell, data_action, present_action = st.columns([7.8, .9, 1.3])
    with top_shell:
        st.markdown(
            f"""
    <div class="sr-top">
    ...
    with data_action:
        if st.button(
            "Data",
            key="open_data_workspace",
            use_container_width=True,
            type="primary" if page == "Cases & Data" else "secondary",
        ):
            st.session_state.nav_page = "Cases & Data"
            st.rerun()
    with present_action:
